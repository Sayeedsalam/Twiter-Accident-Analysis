...
county_info = {}
with open("county_info.txt", "r") as f:
    for line in f:
        words = line.split(",")
        county_info[words[0]] = words[1]+","+words[2]+","+words[3]

county_names = list(county_info.keys())
search_query= "social distancing"
...
prev_largest = ["" for x in county_names]
...
def cluster_tweets(new_messages):
    ...
    for message in new_messages:
        ...
        # id += 1
        tweet["cluster_id"]= -1
        db["social_distance_tweets_en_no_county"].insert(tweet)
...
def collect_tweets():
    global prev_largest
    search_q = "social distanc"
    #search_q = "social distancing "+county_names[current_index] + " county"
    s_q =   "stay home "+ county_names[current_index] + " county"
    s_q2 = "shelter "+county_names[current_index]+" county"

    results = authenticate().search(q=search_q, since_id=prev_largest[current_index], lang="en", count=500, tweet_mode="extended")
    return results
...
def process_tweet():
    ...
    for tweet in tweets_attribute:
        ...
        tweet["county"] = county_names[current_index]

    tweets_json = [json.dumps(tweet) for tweet in tweets_attribute]

    cluster_tweets(tweets_json)
    print("county in this step was", county_names[current_index])
    print("clustering done")

    # current_index += 1
    # if current_index == len(county_names):
    #     current_index = 0
