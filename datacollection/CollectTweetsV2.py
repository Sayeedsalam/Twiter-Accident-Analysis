...
def compareResults(results_without_location, results_with_location):  # removes dups
    global radius, largest_id, prev_tweet_ids
    ...
    for tweet in store_this_set:
        new_tweet_ids.append(tweet["id_str"])
        if tweet["id_str"] in prev_tweet_ids:
           #print("Duplicate")
           continue
        send_tweet_to_pipeline(tweet)
        print ("Sent new tweet to pipeline")

    prev_tweet_ids = []
    for tweet_id in new_tweet_ids:
        prev_tweet_ids.append(tweet_id)
...
def search_without_location(searchQuery):  # searches with out a location
    results = authenticate().search(q=searchQuery, lang="en", count=100, tweet_mode="extended", result_type="recent")
    return results
...
def search_with_location(latitude, longitude):  # searches with coordinates and has a radius that is changeable
    ...
    results = authenticate().search(q="Car Accident -filter:retweets", geocode=tweet_search_location, lang="en",
                                    count=100, tweet_mode="extended", result_type="recent")
    return results
