...
def cluster_tweets(new_messages):
    ...
    for message in new_messages:
        ...
        # id += 1
        tweet["cluster_id"]= -1
        db[keywords[current_index]+"_ext"].insert(tweet)
...
def collect_tweets():
    global prev_largest
    results = authenticate().search(q=keywords[current_index], since_id=prev_largest[current_index], lang="en", count=500, tweet_mode="extended")
    return results
